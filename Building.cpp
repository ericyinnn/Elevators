...
void Building::update(Move move){
    if (move.isPassMove()) {
        ...
        int numToMove = move.getNumPeopleToPickup();
        int a = elevators[move.getElevatorId()].getCurrentFloor();
        move.copyListOfPeopleToPickup(newList);
        floors[a].removePeople(newList, numToMove);
    }
