...
bool Game::isValidPickupList(const string& pickupList, const int pickupFloorNum) const {
    int d = 0;
    Floor fl;
    fl = building.getFloorByFloorNum(pickupFloorNum);
    if(pickupList.length() > ELEVATOR_CAPACITY) {
        return false;
    }
    int max = pickupList.at(0) - '0';
    for (int z = 0; z < pickupList.length(); z++) {
        if (max < (pickupList.at(z) - '0')) {
            max = pickupList.at(z) - '0';
        }
    }
    if (max >= fl.getNumPeople()) {
        return false;
    }
    ...
    for (size_t i = 0; i < pickupList.size() - 1; i++) {
        for (size_t j = i + 1; j < pickupList.size(); j++) {
    ...
    Person people;
    bool isGoingup;
    isGoingup = fl.getPersonByIndex(pickupList.at(0) - '0').getTargetFloor() > pickupFloorNum;
    for (int m = 1; m < pickupList.length(); m++) {
        people = fl.getPersonByIndex(pickupList.at(m) - '0');
        bool Direction = people.getTargetFloor() > pickupFloorNum;
